...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        _autoCreateSubConnector: function (operator, connector, connectorType, subConnector) {
            ...
            if (connectorInfos.multiple === undefined) {
        ...
        redrawFlowsLayer: function () {
            console.log(this.data);
            //this._clearLinksLayer();
            for (var flowId in this.data.flows) {
                if (this.data.flows.hasOwnProperty(linkId)) {
                    this._drawflow(linkId);
                }
            }
        },
        ...
        _clearLinksLayer: function () {
            ...
            if (this.options.verticalConnection) {
                this.objs.layers.operators.find('.flowchart-operator-connector-small-arrow').css('background-colour', 'transparent');
            } else {
                this.objs.layers.operators.find('.flowchart-operator-connector-small-arrow').css('background-colour', 'transparent');
            }
        },
        ...
        _drawFlow: function (linkId) {
            var linkData = this.data.flows[linkId];
        ...
        _refreshLinkPositions: function (linkId) {
            ...
            var fromY = fromPosition.y + 8;

            var toX = toPosition.x;
            var toY = toPosition.y  + 8;
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            function addConnector(connectorKey, connectorInfos, $operator_container, connectorType, isFlowConnection) {
                var operator_connector_set_class = isFlowConnection ? 'flowchart-operator-flow-set' : 'flowchart-operator-connector-set'; 
                var $operator_connector_set = $(`<div class="${operator_connector_set_class}"></div>`);
                ...
                if ($.isArray(connectorInfos.label)) {
                    for (var i = 0; i < connectorInfos.label.length; i++) {
                        self._createSubConnector(connectorKey, connectorInfos.label[i], fullElement, isFlowConnection);
                    }
                } else {
                    self._createSubConnector(connectorKey, connectorInfos, fullElement, isFlowConnection);
                }
            }

            if (infos.flows === undefined) {
                infos.flows = { inputs: {}, outputs: {}}
            }

            for (var key_f in infos.flows.inputs) {
                if (infos.flows.inputs.hasOwnProperty(key_f)) {
                    addConnector(key_f, infos.flows.inputs[key_f], $operator_inputs, 'inputs', true );
                }
            }

            for (var key_f in infos.flows.outputs) {
                if (infos.flows.outputs.hasOwnProperty(key_f)) {
                    addConnector(key_f, infos.flows.outputs[key_f], $operator_outputs, 'outputs', true );
                }
            }

            for (var key_i in infos.inputs) {
                if (infos.inputs.hasOwnProperty(key_i)) {
                    addConnector(key_i, infos.inputs[key_i], $operator_inputs, 'inputs', false);
                }
            }

            for (var key_o in infos.outputs) {
                if (infos.outputs.hasOwnProperty(key_o)) {
                    addConnector(key_o, infos.outputs[key_o], $operator_outputs, 'outputs', false);
                }
            }
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement, isFlowConnection) {
            ...
            var conector_class = isFlowConnection ? 'flowchart-operator-connector-arrow' : 'checkmark';
            var sub_conector_class = isFlowConnection ? 'flowchart-operator-connector-small-arrow' : 'checkmark-checked';
            if(isFlowConnection) {
                console.log(conector_class);
                console.log(sub_conector_class);
            }
            ...
            var $operator_connector_arrow = $(`<div class="${conector_class}"></div>`);

            $operator_connector_arrow.appendTo($operator_connector);

            var $operator_connector_small_arrow = $(`<div class="${sub_conector_class}"></div>`);
        ...
        _connectorClicked: function (operator, connector, subConnector, connectorCategory) {
            if (connectorCategory == 'outputs') {
                ...
                var x = position.x + position.width + 6;
                var y = position.y + 6;
        ...
        colorizeLink: function (linkId, color) {
            ...
            linkData.internal.els.rect.setAttribute('fill', color);
                ...
                linkData.internal.els.fromSmallConnector.css('background-color', color);
                linkData.internal.els.toSmallConnector.css('background-color', color);
